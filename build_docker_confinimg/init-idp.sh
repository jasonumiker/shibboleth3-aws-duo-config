...
mkdir -p /ext-mount/customized-shibboleth-idp/conf/authn
mkdir -p /ext-mount/customized-shibboleth-idp/system
mkdir -p /ext-mount/customized-shibboleth-idp/system/messages
...
cp conf/{attribute-resolver.xml,attribute-filter.xml,idp.properties,ldap.properties,metadata-providers.xml,relying-party.xml} /ext-mount/customized-shibboleth-idp/conf/
cp conf/authn/{duo.properties,mfa-authn-config.xml} /ext-mount/customized-shibboleth-idp/conf/authn
cp system/messages/messages.properties /ext-mount/customized-shibboleth-idp/system/messages
